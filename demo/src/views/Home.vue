 ...
 * @LastEditTime: 2026-09-21 12:43:32
 * @Description: 
-->
<template>
    <div class="home">
        <h1>{{ title }}</h1>
        <el-button type="primary" @click="$store.commit('increment')">count = {{ $store.state.count }}</el-button>
        ...
        <div class="cards">
            <user-card v-for="u in users" :key="u.id" :name="u.name" :age="u.age" :active="u.id === activeId" @select="activeId = u.id" />
        </div>

        <h2>el-table 用户管理</h2>
        <div class="toolbar">
            <el-input v-model="search" placeholder="搜索姓名 / 邮箱" clearable prefix-icon="el-icon-search" style="width: 240px" />
        ...
        <el-pagination class="pager" layout="total, prev, pager, next" :total="filtered.length" :page-size="pageSize" :current-page.sync="currentPage" />

        <el-dialog :visible.sync="dialogVisible" :title="form.id ? '编辑用户' : '新增用户'" width="460px">
        ...
        <h2>el-tree</h2>
        <el-tree :data="treeData" :props="treeProps" default-expand-all node-key="id" style="width: 480px" />
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        filtered() {
            ...
            return this.tableUsers.filter(u => u.name.includes(kw) || u.email.includes(kw));
        },
        pagedUsers() {
