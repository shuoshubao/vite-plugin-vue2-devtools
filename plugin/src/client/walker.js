...
function displayName(vm) {
  ...
  if (!name && opts.__file) {
    ...
    name = String(opts.__file).split(/[\\/]/).pop().replace(/\.vue$/, '')
  }
...
export function buildTree() {
  registry.clear()
  return findRoots().map((vm) => walk(vm))
}
...
// Extract props / data / computed for the detail pane.
export function inspect(vm) {
  ...
  if (propDefs) {
    for (const key of Object.keys(propDefs)) {
      props.push({ key, value: formatValue(vm[key]) })
    }
  }
  ...
  if (raw) {
    for (const key of Object.keys(raw)) {
      data.push({ key, value: formatValue(raw[key]) })
    }
  }
  ...
  if (computedDefs) {
    for (const key of Object.keys(computedDefs)) {
      let value
      try {
        value = formatValue(vm[key]) // reading the getter is intentional
      } catch (err) {
