...
export class VueDevToolsPanel extends LitElement {
    ...
    _persistUiState() {
        try {
            ...
            localStorage.setItem(STORE_KEY, JSON.stringify({ collapsed: this.collapsed, tab: this.tab, theme: this.theme, pos }));
        } catch (e) {
    ...
    _index(filter) {
        ...
        if (filter) {
            const walk = (n, p) => {
                ...
                parent.set(n.id, p);
                order.push(n.id);
                if (this._searchCollapsed.has(n.id)) return; // collapsed in search
    ...
    _handleKeydown(e) {
        ...
        if (this.tab === 'timeline') {
            ...
            const curIdx = entries.findIndex(x => x.id === this.timelineSelected);
            const base = curIdx < 0 ? entries.length - 1 : curIdx;
            const nextIdx = e.key === 'ArrowDown' ? Math.min(entries.length - 1, base + 1) : Math.max(0, base - 1);
        ...
        if (e.key === 'ArrowDown') {
            ...
            if (hasChildren && !isOpen) searching ? this._searchCollapsed.delete(id) : this.expanded.add(id);
            else if (hasChildren && isOpen) next = shownChildren[0].id;
        } else if (e.key === 'ArrowLeft') {
            // Check open/closed FIRST: open -> collapse; closed (or leaf) -> parent.
            if (isOpen && hasChildren) searching ? this._searchCollapsed.add(id) : this.expanded.delete(id);
            ...
            <div>
                ...
                >
                    <span
                        ...
                        @click=${e => {
                            e.stopPropagation();
                            if (hasKids) this._toggleSearchNode(node.id);
                        }}
                    >
                ...
                ${hasKids && open ? kids.map(c => this._renderSearchNode(c, depth + 1, show)) : null}
            </div>
        `;
    }

    _toggleSearchNode(id) {
        if (this._searchCollapsed.has(id)) this._searchCollapsed.delete(id);
        else this._searchCollapsed.add(id);
        this.requestUpdate();
    }
    ...
    _renderKvSection(title, obj, editable) {
        ...
        const showFilter = !collapsed && allKeys.length > SECTION_FILTER_THRESHOLD;
